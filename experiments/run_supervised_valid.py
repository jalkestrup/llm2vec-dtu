...
class DataTrainingArguments:
    ...
    validation_file_path: Optional[str] = field(
        default=None, metadata={"help": "The input validation data file or folder."}
    )
    # TODO: implement this
    ...
    metadata={
        "help": (
            "For debugging purposes or quicker evaluation, truncate the number of validation examples to this "
            "value if set."
        ),
    },
    )
...
def limit_dataset(dataset, max_samples):
    if len(dataset) > max_samples:
        ...
        # Select the first `max_samples` indices
        selected_indices = indices[:max_samples]
        
        # Extract the subset corresponding to the selected indices
...
def main():
    ...
    val_dataset = load_dataset(
        data_args.dataset_name,
        split="train", #On purpose, the train has no effect as we are loading a specific validation path
        file_path=data_args.validation_file_path,
        effective_batch_size=training_args.per_device_eval_batch_size
        * accelerator.num_processes,
    )

    if data_args.max_eval_samples is not None:
        val_dataset = limit_dataset(val_dataset, data_args.max_eval_samples)
    ...
    # Debugging: print content to ensure structure
